...
class Sample:
  ...
  def plot_landscape_scatter(self):
      ...
      xmax = np.nanmax(self.cell_data.x)
      ymax = np.nanmax(self.cell_data.y)
      j = 0
      for i, row in self.classes.iterrows():
          aux = self.cell_data.loc[self.cell_data['class'] == row['class']]
          if (len(aux) > 0):
  ...
  def plot_landscape_simple(self):
      ...
      with warnings.catch_warnings():
          ...
          landscapeLevels(ax[1], x, y, z, m, levs,
                          self.units, xedges, yedges, fontsiz=18)
          ax[1].set_title('Tumor cells - KDE levels', fontsize=18, y=1.02)
      ...
      with warnings.catch_warnings():
          ...
          for i, row in self.classes.iterrows():
              ...
              ax[i, 0].set_title('Log Abundance image: ' + name,
                                 fontsize=18, y=1.02)
              ax[i, 0].set_xlim([0, abuim.shape[0]])
              ax[i, 0].set_ylim([0, abuim.shape[1]])
              ...
              ax[i, 1].set_xlim([0, mixim.shape[0]])
              ax[i, 1].set_ylim([0, mixim.shape[1]])
...
def main(args):
    ...
    *******  Main function  *******
    ...
    if  (REDO or 
        ...
        extent = [0, sample.imshape[0]*sample.scale,
                  0, sample.imshape[1]*sample.scale]
