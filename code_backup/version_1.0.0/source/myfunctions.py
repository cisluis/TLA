...
def landscapeLevels(ax, x, y, z, m, levs, units, xedges, yedges,
    ...
    cbar = plt.colorbar(CS1, cax=axins, format=FuncFormatter(fmt_simple))
...
def morisita_horn_univariate(x, min_x=5, min_b=3):
    ...
    if sum(x) > 0:
        ...
        if (len(z) >= min_b):
            ...
            m = morisita_horn(z, np.ones(len(z)))
    return(m)
...
def getis_ord_g_array(X, msk):
    ...
    Parameters
    ----------
    - N: array of data containing counts of events per kernel.
    - msk: ROI mask
    """
...
def comp_correlations(raster, comps, classes, ttl, lims):
    ...
    for i, compi in enumerate(comps):
        auxi = raster[:, :, compi[0], compi[1]].ravel()
        for j, compj in enumerate(comps):
            if j > i:
                ij = np.unravel_index(k, shp)
                auxj = raster[:, :, compj[0], compj[1]].ravel()
                ...
                xlab = '(' + classes.iloc[compi[0]].class_name + \
                    ':' + classes.iloc[compi[1]].class_name + ')'
                ylab = '(' + classes.iloc[compj[0]].class_name + \
...
def factor_correlations(rasteri, comps1, tti, limsi,
    ...
    for i, compi in enumerate(comps1):
        auxi = rasteri[:, :, compi[0], compi[1]].ravel()
        for j, compj in enumerate(comps2):
            ij = np.unravel_index(k, shp)
            auxj = rasterj[:, :, compj[0], compj[1]].ravel()
            ...
            xlab = tti + '(' + classes.iloc[compi[0]].class_name + \
                ':' + classes.iloc[compi[1]].class_name + ')'
            ylab = ttj + '(' + classes.iloc[compj[0]].class_name + \
                ':' + classes.iloc[compj[1]].class_name + ')'
