...
class Landscape:
  ...
  def loadLMELandscape(self, redo, do_plots=False):
      ...
      if (redo \
          ...
          for i, iclass in self.classes.iterrows():
              ...
              lims[i][0] = np.ceil(nedges[i][1])
              lims[i][1] = round((np.ceil(nedges[i][2])+lims[i][0])/20)*20
              ...
              # removes values lower than lower limit
              aux = abuarr[:, :, i]
              aux[aux < lims[i][0]] = np.nan  
              abuarr[:, :, i] =  aux[:, :]
          ...
          if (redo or not os.path.exists(fclt)):
              ...
              for i, iclass in self.classes.iterrows():
                  ...
                  # makes a rater for cell clusters 
                  # (mass with abundance in top 87.5%)
                  aux = cltarr[:, :, i]
                  aux[aux < lims[i][1]] = np.nan
                  aux[aux >= lims[i][1]] = 1  
                  cltarr[:, :, i] =  aux[:, :]
                  
                  # create pylandscape object with binned factor values
                  pl = pls.Landscape(aux, res=(1, 1), neighborhood_rule='8')
                  if (len(pl.classes) > 0):
  ...
  def plotGeOrdLandscape(self, redo, analyses, lims, do_plots=False):
      """
  ...
  def landscapeAnalysis(self, sample, redo, do_plots=False):
      
      samplcsv = os.path.join(sample['res_pth'], self.sid +'_lme_tbl.csv')
...
def plotCompLandscape(sid, raster, roi, comps, compsterm, compslabs, shape,
                      metric, lims_in, scale, units, binsiz, 
    ...
    with warnings.catch_warnings():
        ...
        for i, comp in enumerate(comps):
            ...
            bins = np.round(np.arange(lims[0], 
                                      lims[1] + bini, bini), 3)
            cticks = np.round(np.arange(lims[0], 
                                        lims[1] + 2*bini, 2*bini), 3)
...
def main(args):
    ...
    *******  Main function  *******
    ...
    if debug:
        ...
        argsfile = os.path.join(main_pth, 'PC_all.csv')
        REDO = True
        GRPH = True
        CASE = 0
    else:
    ...
    if (sample.num_cells > 0):
        ...
        # %% STEP 2: create landscape object
        ...
        land.loadLMELandscape(REDO, GRPH)
        ...
        land.landscapeAnalysis(sample, REDO, GRPH)      
